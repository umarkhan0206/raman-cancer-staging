...
RESULTS_DIR = '/users/vvgk0135/raman-project/results/2026_12ch_groupnorm'
...
print(f'GroupNorm only | Epochs: {NUM_EPOCHS} | Patience: {PATIENCE}')
...
print("Computing protein max from training sessions only...")
protein_max = 0.0
for cl in CLASS_MAP:
...
print(f"Protein max (train only): {protein_max:.2f}")
...
def build_model():
    ...
    model.conv1 = nn.Conv2d(N_CH, 64, kernel_size=7, stride=2, padding=3, bias=False)
    model.fc = nn.Sequential(nn.Dropout(p=0.3), nn.Linear(model.fc.in_features, 4))
    model = convert_bn_to_gn(model, num_groups=8)
    bn_left = sum(1 for m in model.modules() if isinstance(m, nn.BatchNorm2d))
    gn_count = sum(1 for m in model.modules() if isinstance(m, nn.GroupNorm))
    print(f'  {N_CH}ch ResNet50 GroupNorm | BN remaining: {bn_left} | GN layers: {gn_count} | params: {sum(p.numel() for p in model.parameters()):,}')
...
print(f'\n{"="*60}')
print(f'FINAL SUMMARY — 12ch GroupNorm, protein_max, TTA')
print(f'{"="*60}')
