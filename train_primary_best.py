...
RESULTS_DIR = '/users/vvgk0135/raman-project/results/2026_12ch_pretrained'
...
print(f'Pretrained ResNet50 | Epochs: {NUM_EPOCHS} | Patience: {PATIENCE}')
...
def build_model():
    ...
    # Replace first conv: 3ch -> 12ch
    # Initialise by averaging the 3 pretrained input channels across 12
    old_conv = model.conv1
    new_conv = nn.Conv2d(N_CH, 64, kernel_size=7, stride=2, padding=3, bias=False)
    with torch.no_grad():
        # Average the 3 RGB channels and repeat across 12 channels
    ...
    print(f'  {N_CH}ch ResNet50 pretrained (ImageNet) | params: {trainable:,}')
...
print(f'\n{"="*60}')
print(f'FINAL SUMMARY — 12ch Pretrained, protein_max, TTA')
print(f'{"="*60}')
